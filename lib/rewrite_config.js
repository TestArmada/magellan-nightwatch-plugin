/* eslint complexity: 0, global-require: 0, camelcase: 0 */
"use strict";
var fs = require("fs");
...
module.exports = function (sourceConfigPath, tempAssetPath, options, opts) {
  var _opts = {
    console: console,
    fs: fs,
    path: path,
  ...
  var conf = _opts.require(_opts.path.resolve(sourceConfigPath));
  ...
  */
  if (options.sauceSettings) {
    ...
    if (!options.sauceBrowserSettings) {
      _opts.console.error(
        "ERROR: Missing browser settings even though sauce cloud settings are active");
    ...
    if (options.sauceSettings.tunnelId) {
      conf.test_settings.sauce.desiredCapabilities["tunnel-identifier"] =
        options.sauceSettings.tunnelId;
      if (options.sauceSettings.sharedSauceParentAccount) {
        // if tunnel is shared by parent account
        conf.test_settings.sauce.desiredCapabilities["parent-tunnel"] =
          options.sauceSettings.sharedSauceParentAccount;
      }
    } else {
  ...
  var tempConfigPath = _opts.path.resolve(tempAssetPath + "/nightwatch.json");

  _opts.fs.writeFileSync(tempConfigPath, JSON.stringify(conf), "utf8");
