...
const setupScreenshotDir = function() {
  console.log(`Filepath for this file: ${__filename}`);

  const desiredPath = process.env.NIGHTWATCH_SCREENSHOT_PATH || "";
  try {
    fs.mkdirSync(desiredPath);
    return desiredPath;
  } catch (err) {
    console.log(`Error: ${err.code} for path: ${desiredPath}`);
    if (err.code === "EEXIST") {
      return desiredPath;
    }
    console.log(
