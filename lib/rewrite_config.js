...
module.exports = function (sourceConfigPath, tempAssetPath, options) {
  ...
  if (options.isFixingChromedriver) {
    ...
    if (conf.selenium.cli_args["webdriver.chrome.driver"] !== null) {
  ...
  if (options.isFixingChromedriver) {
    ...
    if (conf.test_settings.phantomjs.desiredCapabilities["phantomjs.binary.path"] !== null) {
      // don't overwrite user value
