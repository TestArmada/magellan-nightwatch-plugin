...
module.exports = function (sourceConfigPath, tempAssetPath, options) {
  ...
  var newsourceConfigPath = sourceConfigPath.replace('test/automation/conf/','');
  ...
  if (path.extname(tempConfigPath) === '.js') {
    prependFile(tempConfigPath, 'module.exports = ', function(err) {
      if (err) {
        console.log(err);}
      console.log('Convert to .js format if the orginal nightwatch config is .js format.');
    });
  }
