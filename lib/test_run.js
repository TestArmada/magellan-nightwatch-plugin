...
NightwatchTestrun.prototype.enableExecutor = function (executor) {
  this.sauceSettings = executor.getConfig();
...
NightwatchTestrun.prototype._createTemporaryConfigFile = function () {
  ...
  if (this.profile.nightwatchEnv === "sauce") {
    ...
    if (!sauceSettings.useTunnels && !sauceSettings.sauceTunnelId) {
      ...
      delete sauceSettings.sauceTunnelId;
    }

    nightwatchConfigOptions.sauceSettings = sauceSettings;
    nightwatchConfigOptions.desiredCapabilities = this.profile.desiredCapabilities;
