...
var NightwatchTestrun = function (options, _opts) {
  this.opts = {};
  ...
  /* istanbul ignore next */
  if (!this.opts.rewriteNightwatchConfig) {
    this.opts.rewriteNightwatchConfig = rewriteNightwatchConfig;
  }

  _.extend(this, options);
  nightwatchConfigPath = path.normalize(this.opts.settings.nightwatchConfigFilePath);
...
NightwatchTestrun.prototype._createTemporaryConfigFile = function () {
  ...
  if (this.sauceBrowserSettings) {
    ...
    nightwatchConfigOptions.localSeleniumVersion = this.opts.settings.seleniumServerVersion;
    nightwatchConfigOptions.isChromedriverPresent = this.opts.settings.isChromedriverPresent;
    nightwatchConfigOptions.isPhantomjsPresent = this.opts.settings.isPhantomjsPresent;
  }

  // throws exceptions
  this.configPath = this.opts.rewriteNightwatchConfig(nightwatchConfigPath, this.tempAssetPath,
    nightwatchConfigOptions, this.opts);
};
...
NightwatchTestrun.prototype.getArguments = function () {
  var args = [
    ...
    // If we're not in sauce mode, we just pass in a browserId directly
    // (i.e. phantomjs, chrome, firefox, etc)
  ...
  if (this.opts.settings.aggregateScreenshots) {
